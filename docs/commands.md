...
- `/roster` is supplied in NFL workspaces. The agent runs the current bundled roster script through its existing `exec` tool and explains the fresh roster. `/roster show only my bench` supplies an additional request. This uses the configured chat model for execution decisions and presentation; the script itself makes no model calls. Customized workflows keep their own saved instructions.
- `/fantasy-update` refreshes validated NFL fantasy totals, rankings, starter and bench scores. Use `--week N` for an explicitly requested week. Commissioner overrides, including zero, are retained. Ties have no winner; current-week results show leaders. A winner is reported only once the NFL state has advanced beyond that week.
...
The app seeds registration instructions and the NFL roster/fantasy workflows when commands are first loaded, including for existing chats. Known default definitions and scripts are routed to the current app-owned bundle under `.harness/workflows/<content-hash>/`, so code updates reach existing workspaces. The command menu and run prompt identify that source. Existing workspace definitions and scripts are not overwritten; customized workflows continue using their own instructions. Previous bundles identify older untouched defaults. Modified bundle files produce an error rather than silently running altered code. Workers can propose definitions and scripts as artifacts for the parent to install in the shared workspace.
...
**Refresh data** calls the same deterministic Sleeper refresh endpoint as My matchup, without a model call or chat message. It preserves the draft, trade selections, and filters. Failures leave the previous display and timestamp visible with an error. The panel rereads saved files when opened and after chat runs. On narrow screens, Teams opens over the chat; adding a discussion draft closes the panel and focuses the composer. Missing configuration, missing snapshots, unreadable data, and missing player names have separate messages. NBA chats cannot access the NFL endpoint; their Teams and Waivers views show the unconnected ESPN Fantasy state.
...
League and My matchup use the same newest validated score snapshot, season/week, team totals, and player scores. The readers accept both `latest.json` and `fantasy-latest.json`. Missing player scores can fall back to another snapshot of that same season/week, with their individual retrieval times retained. Matchup lineups and opponent IDs come from the selected score snapshot. Historical `--week` results are labeled with their saved week and do not borrow points from another week.

The server caches the parsed player catalog for up to four workspace roots and shares concurrent reads. Every request still checks the resolved path, size, file identity, and modification/change times. Replacement or modification invalidates the cache; deleted or invalid files do not silently serve the previous catalog. In a local 15-read benchmark, warm median roster reads fell from 41 ms to 0.9 ms and waiver reads from 44 ms to 2 ms. Cold reads still parse the catalog; these are local reader measurements, not end-to-end latency guarantees.
